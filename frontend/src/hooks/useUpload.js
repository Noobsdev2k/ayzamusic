...
import { uploadMusicStore } from "../store/reducer/upload";
...
export const UseUploadMusic = () => {
  ...
  const [newAudio, setNewAudio] = React.useState(false);
  // store
  const resultUploadMusic = useSelector(uploadMusicStore);
  ...
  const handleGetUploadMusic = React.useCallback(
    (params) => accessToken && dispatch(getUploadMusic(params)),
    [dispatch, accessToken]
  );

  const handlePostSourceMusic = React.useCallback(
    (data) => accessToken && dispatch(postUploadMusic(data)),
    [dispatch, accessToken]
  );

  const handleEditMusicUpload = React.useCallback(
    (data) => accessToken && dispatch(editUploadMusic(data)),
    [dispatch, accessToken]
  );

  const handleDeleteMusic = React.useCallback(
    (params) => accessToken && dispatch(deleteMusic(params)),
    [dispatch, accessToken]
  );
  ...
  const handlePostUploadMusic = async (value) => {
    if (value) {
      ...
      formData.append(
        "image_music",
        image_music ? image_music[0].originFileObj : others?.image_music
      );
      formData.append(
        "src_music",
        src_music ? src_music[0].originFileObj : others?.src_music
      );
      ...
      if (youtube.length === 4) {
        if (others) {
          ...
          if (result) toggle({ type: ModalType.NULL, title: "" });
